...
func (n *TestAgent) Start(id TransactionID, deadline time.Time, f Handler) error {
	n.f <- f
	return nil
}
...
func TestClient_Do(t *testing.T) {
	response := MustBuild(TransactionIDSetter, BindingSuccess)
	response.Encode()
	conn := &testConnection{
		b: response.Raw,
	...
	defer func() {
		...
		if err := c.Do(MustBuild(TransactionIDSetter), time.Time{}, nil); err == nil {
	...
	m = MustBuild(TransactionIDSetter)
...
func (a errorAgent) Start(id TransactionID, deadline time.Time, f Handler) error {
	return a.startErr
}
...
func (a *gcWaitAgent) Stop(id TransactionID) error {
	return nil
}
...
func (a *gcWaitAgent) Start(id TransactionID, deadline time.Time, f Handler) error {
	return nil
}
