...
// NewTransactionID sets m.TransactionIDSetter to random value from crypto/rand
...
//for debug only
func getAttrIntercepter(attr AttrType, value []byte, m *Message) string {
	/*
						const (
	...
	*/
	...
	case AttrRealm:
		fallthrough
	case AttrSoftware:
		fromer = &TextAttribute{}
	case AttrErrorCode:
		fromer = new(ErrorCodeAttribute)
	}
	if fromer == nil {
		return hex.EncodeToString(value[:])
	}
	err := fromer.GetFromAs(m, attr)
	if err == nil {
		return fromer.String()
	}
	return hex.EncodeToString(value[:])
}
func (m *Message) String() string {
	buf := new(bytes.Buffer)
	buf.WriteString(fmt.Sprintf("%s %s\n", m.Type.Method, m.Type.Class))
	buf.WriteString(fmt.Sprintf("Header: length=%d,tx_id=%s\n", m.Length, hex.EncodeToString(m.TransactionID[:])))
	buf.WriteString("Attributes:\n")
	for _, a := range m.Attributes {
		buf.WriteString(fmt.Sprintf("%s: %s\n", a.Type.String(), getAttrIntercepter(a.Type, a.Value, m)))
	}
	return string(buf.Bytes())
	//return fmt.Sprintf("%s len=%d attrs=%d txid=%s",
	//	m.Type,
	//	m.Length,
	//	len(m.Attributes),
	//	base64.StdEncoding.EncodeToString(m.TransactionIDSetter[:]),
	//)
}
...
// WriteTransactionID writes m.TransactionIDSetter to m.Raw.
...
var (
	...
	/*
		Binding indication message type,to keep stun valid,
	*/
...
const (
	...
	MethodChannelData      Method = 0x4000 //need trick for turn data message. todo 必须特殊处理.
)
...
// to provided value.
func NewTransactionIDSetter(value TransactionID) Setter {
