...
import (
	...
	"github.com/SmartMeshFoundation/SmartRaiden/network/rpc/contractsv203/contracts"
...
func setup() {
	...
	tokenNetworkAddress = common.HexToAddress("0x5e6dc7fa2d8b888f8a4aaea31e49c9ffd8b0d3d9")
...
func creatAChannelAndDeposit(account1, account2 common.Address, key1 *ecdsa.PrivateKey, amount int64, conn *ethclient.Client) error {
	...
	tx, err = tokenNetwork.SetTotalDeposit(auth1, account2, account1, big.NewInt(amount))
	if err != nil {
...
func getTestOpenChannel(t *testing.T) (channelID contracts.ChannelIdentifier, partnerAddr common.Address, partnerKey *ecdsa.PrivateKey, err error) {
	...
	channelID, settleBlockNumber, openBlockNumber, state, err = tokenNetwork.GetChannelInfo(nil, auth.From, partnerAddr)
	if err != nil {
		t.Error(err)
		return
	}
...
func TestCloseChannel1(t *testing.T) {
	...
	tx, err := tokenNetwork.CloseChannel(auth, partnerAddr, utils.BigInt0, utils.EmptyHash, utils.BigInt0, utils.EmptyHash, nil)
	if err != nil {
		t.Error(err)
	}
	r, err := bind.WaitMined(context.Background(), client, tx)
...
func TestCloseChannelAndUpdateBalanceProofDelegateAndSettle(t *testing.T) {
	...
	tx, err = tokenNetwork.UpdateBalanceProofDelegate(auth, auth.From, partnerAddr, bp2.TransferAmount, bp2.LocksRoot, bp2.Nonce, utils.BigInt0, utils.EmptyHash, utils.BigInt0, bp2.AdditionalHash, bp2.Signature, bp2.NonClosingSignature)
	if err != nil {
		t.Error(err)
		return
	}
	...
	log.Info(fmt.Sprintf("SettleChannel arg p1=%s,p2=%s,p1.transferAmount=%s,"+
		"p2.transferAmount=%s,p1.locksroot=%s,p2.locksroot=%s,p1.nonce=%s,p2.nonce=%s,"+
...
func TestCloseChannelAndUpdateBalanceProofAndSettle(t *testing.T) {
	...
	*/
	...
	tx, err = tokenNetwork.UpdateBalanceProof(partnerAuth, auth.From, bp2.TransferAmount, bp2.LocksRoot, bp2.Nonce, utils.BigInt0, utils.EmptyHash, utils.BigInt0, bp2.AdditionalHash, bp2.Signature)
	if err != nil {
		t.Error(err)
		return
	}
	...
	log.Trace(fmt.Sprintf("SettleChannel arg,p1=%s,p1.amount=%s,p1.lock=%s,p1.nonce=%s,p2=%s,p2.amount=%s,p2.lock=%s,p2.nonce=%s",
...
func TestUnlock(t *testing.T) {
	...
	tx, err = tokenNetwork.Unlock(auth, auth.From, partnerAddr, bp2.TransferAmount, bp2.LocksRoot, bp2.Nonce, lockbytes)
	if err != nil {
		t.Error(err)
		return
	}
	...
	tx, err = tokenNetwork.SettleChannel(
		...
		bp2.TransferAmount.Add(bp2.TransferAmount, big.NewInt(int64(maxLocks))),
		bp2.LocksRoot,
		bp2.Nonce,
	)
	if err != nil {
...
func TestPunishObsoleteUnlock(t *testing.T) {
	...
	log.Info(fmt.Sprintf("unlockarg,channelid=%s,partnerAddr=%s,part2=%s,locksroot=%s", common.Hash(channelID).String(), partnerAddr.String(),
		auth.From.String(), ou.LocksRoot.String()))
