...
/*
此版本是在19版本基础之上
主要考虑 如何节省 gas,通过参数传递的方式要比存储在合约中节省不少 gas.
另外,处于节省 gas 的考虑,允许 close channel 一方调用 updatetransfer, 为什么不允许呢?
*/
   ...
   */
    ...
    */
    ...
    public
    {
        ...
        if (nonce > 0) {
            ...
            partner_state.balance_hash = calceBalanceHash(nonce, locksroot, transferred_amount);
        }
    ...
    */
    ...
    public
    {
        ...
        Participant storage participant_state = channel.participants[participant];
        participant_state.balance_hash = calceBalanceHash(nonce, locksroot, transferred_amount);
        emit BalanceProofUpdated(channel_identifier, participant, nonce);
    }
    /*
    ...
    */
    ...
    public
    {
        ...
        require(balance_hash == calceBalanceHash(nonce, locksroot, transferered_amount));
        /*
        会不会溢出呢? 两人持续交易?
        正常来说,不会,
        ...
        */
        participant_state.balance_hash = calceBalanceHash(nonce, locksroot, transferered_amount);
    ...
    */
    ...
    public
    {
        ...
        */
        ...
        require(balance_hash == calceBalanceHash(beneficiary_nonce, locksroot, beneficiary_transferred_amount));
        /*
        punish the cheater.
        */
    ...
    */
    ...
    public
    {
        ...
        验证提供的参数是有效的
        */
        require(participant1_state.balance_hash == calceBalanceHash(participant1_nonce, participant1_locksroot, participant1_transferred_amount));
        require(participant2_state.balance_hash == calceBalanceHash(participant2_nonce, participant2_locksroot, participant2_transferred_amount));
    ...
    */
    ...
    view internal
    {
        Channel storage channel = channels[channel_identifier];
        Participant storage participant_state = channel.participants[participant];
        bytes32 balance_hash = calceBalanceHash(nonce, locksroot, transferred_amount);
        require(participant_state.balance_hash == balance_hash);
        require(new_nonce > nonce);
    }

    function calceBalanceHash(uint256 nonce, bytes32 locksroot, uint256 transferred_amount) pure internal returns (bytes32){
