...
contract TokensNetwork is Utils {
    ...
    uint64 constant public punish_block_number = 257; // 3600/14;  //about one hour
    ...
    mapping(address => bool) public registered_token;
    ...
    {
        require(_chain_id > 0);
        secret_registry = new SecretRegistry();
        chain_id = _chain_id;
    }
    function validAndUpdateToken(address token) internal {
        require(token != 0x0);
        require(contractExists(token));
        Token token_contract = Token(token);
        require(token_contract.totalSupply() > 0);
        registered_token[token] = true;
        emit TokenNetworkCreated(token);
    }
...
*/
    ...
    function deposit(address token, address participant, address partner, uint256 amount, uint64 settle_timeout)
    external
    {
        depositInternal(token, participant, partner,msg.sender, amount, settle_timeout, true);
    }
    ...
    */
    ...
    /// @param participant  The address where tokens to be deposited into.
    /// @param partner      The address for the counterpart of participant.
    function depositInternal(address token, address participant, address partner,address from, uint256 amount, uint64 settle_timeout, bool need_transfer)
    internal
    {
        bytes32 channel_identifier = getChannelIdentifier(token, participant, partner);
        ...
        if (channel.state == 1) {
            ...
            // make sure not overflow, test equal because of first deposit is equal
            ...
            emit ChannelOpenedAndDeposit(token, participant, partner, settle_timeout, amount);
        } else {
            //正在结算的通道是不允许存钱的
            revert();
        }
        if (need_transfer) {
            Token token_contract = Token(token);
            // Do the transfer
            require(token_contract.transferFrom(from, address(this), amount));
        }
    }
    ...
    */
    ...
    function tokenFallback(address /*from*/, uint value, bytes data) external returns (bool success){
        fallback(msg.sender, 0, value, data, false);
        return true;
    }
    ...
    function receiveApproval(address from, uint256 value, address token_, bytes data) external returns (bool success) {
        fallback(token_, from, value, data, true);
        return true;
    }


    function fallback(address token, address from, uint256 value, bytes data, bool need_transfer) internal {
        address participant;
        address partner;
        uint64 settle_timeout;
        (participant, partner, settle_timeout) = getOpenWithDepositArg(data);
        depositInternal(token, participant, partner,from, value, settle_timeout, need_transfer);
    ...
    */
    ...
    public
    {
        ...
        */
        ...
        channel.open_block_number = uint64(block.number);
        require(token_contract.transfer(participant, participant_withdraw));
    ...
    */
    ...
    public
    {
        ...
        channel_identifier = getChannelIdentifier(token,msg.sender, partner);
        Channel storage channel = channels[channel_identifier];
        require(channel.state == 1);
    ...
    */
    ...
    public
    {
        bytes32 channel_identifier;
        uint64 settle_block_number;
        channel_identifier = getChannelIdentifier(token,partner, participant);
        Channel storage channel = channels[channel_identifier];
        Participant storage partner_state = channel.participants[partner];
   ...
   */
    ...
    {
        bytes32 channel_identifier;
        channel_identifier = getChannelIdentifier(token,partner, msg.sender);
        Channel storage channel = channels[channel_identifier];
        Participant storage partner_state = channel.participants[partner];
        require(channel.state == 2);
    ...
    */
    ...
    function unlockDelegate(
        ...
        unlockInternal(token,partner, participant, transferred_amount, expiration, amount, secret_hash, merkle_proof);
    }
    ...
    */
    ...
    {
        unlockInternal(token,partner, msg.sender, transferred_amount, expiration, amount, secret_hash, merkle_proof);
    }
    ...
    */
    ...
    public
    {
        ...
        bytes32 lockhash_hash;
        channel_identifier = getChannelIdentifier(token,beneficiary, cheater);
        Channel storage channel = channels[channel_identifier];
        require(channel.state == 2);
    ...
    */
    ...
    public
    {
        ...
        bytes32 channel_identifier;
        Token token_contract=Token(token);
        channel_identifier = getChannelIdentifier(token,participant1, participant2);
        Channel storage channel = channels[channel_identifier];
        ...
        */
        ...
        if (participant1_amount > 0) {
            require(token_contract.transfer(participant1, participant1_amount));
        }

        if (participant2_transferred_amount > 0) {
            require(token_contract.transfer(participant2, participant2_transferred_amount));
        }
    ...
    */
    ...
    public
    {
        ...
        if (participant1_balance > 0) {
            require(token_contract.transfer(participant1, participant1_balance));
        }

        if (participant2_balance > 0) {
            require(token_contract.transfer(participant2, participant2_balance));
        }
    ...
    view
    external
    returns (uint64, uint64, uint8, uint64)
    {
        Channel storage channel = channels[channel_identifier];
    ...
    function getChannelParticipantInfo(address token, address participant, address partner)
    view
    external
    ...
    function queryUnlockedLocks(address token, address participant, address partner, bytes32 lockhash)
    view
    external
    returns (bool)
    {
