...
import (
	...
	"github.com/SmartMeshFoundation/SmartRaiden/network/rpc/contractsv15/contracts"
...
func setup() {
	...
	tokenNetworkAddress = common.HexToAddress("0xfeb0696f7c6a5da2cd925df83ed9fdb68a6c27dc")
...
func creatAChannelAndDeposit(account1, account2 common.Address, key1 *ecdsa.PrivateKey, amount int64, conn *ethclient.Client) error {
	...
	channelID, _, settleBlock, _, state, err := tokenNetwork.GetChannelInfo(nil, account1, account2)
	if err != nil {
...
func getTestOpenChannel(t *testing.T) (channelID *big.Int, partnerAddr common.Address, partnerKey *ecdsa.PrivateKey, err error) {
	...
	channelID, _, settleBlockNumber, _, state, err := tokenNetwork.GetChannelInfo(nil, auth.From, partnerAddr)
	if err != nil {
		t.Error(err)
		return
	}
...
func TestCloseChannel1(t *testing.T) {
	...
	tx, err := tokenNetwork.CloseChannel(auth, channelID, utils.BigInt0, utils.EmptyHash, utils.BigInt0, utils.BigInt0, utils.EmptyHash, nil)
	if err != nil {
		t.Error(err)
	}
	r, err := bind.WaitMined(context.Background(), client, tx)
...
func (b *BalanceProofForContract) sign(key *ecdsa.PrivateKey) {
	...
	buf.Write(utils.BigIntTo32Bytes(b.WithdrawNonce))
...
func TestCloseChannel2(t *testing.T) {
	...
	tx, err := tokenNetwork.CloseChannel(auth, channelID, bp.TransferAmount, bp.LocksRoot, bp.Nonce, bp.WithdrawNonce, bp.AdditionalHash, bp.Signature)
	if err != nil {
		t.Error(err)
	}
	r, err := bind.WaitMined(context.Background(), client, tx)
...
func TestCloseChannelAndUpdateNonClosingAndSettle(t *testing.T) {
	...
	tx, err = tokenNetwork.UpdateNonClosingBalanceProof(auth, channelID, partnerAddr, bp2.LocksRoot, bp2.TransferAmount, bp2.Nonce, bp2.WithdrawNonce, bp2.AdditionalHash, bp2.Signature, bp2.NonClosingSignature)
	if err != nil {
		t.Error(err)
		return
	}
	...
	_, _, blokNumber, _, state, err := tokenNetwork.GetChannelInfo(nil, auth.From, partnerAddr)
	if err != nil {
		t.Error(err)
		return
	}
	...
	log.Info(fmt.Sprintf("SettleChannel  complete.. gasLimit=%d,gasUsed=%d", tx.Gas(), r.GasUsed))
}
...
func TestCloseChannelAndUpdateNonClosingAndSettle2(t *testing.T) {
	...
	//第二次 update, 应该也要成功
	tx, err = tokenNetwork.UpdateNonClosingBalanceProof(auth, channelID, partnerAddr, bp3.LocksRoot, bp3.TransferAmount, bp3.Nonce, bp3.WithdrawNonce, bp3.AdditionalHash, bp3.Signature, bp3.NonClosingSignature)
	if err != nil {
		t.Error(err)
		return
	}
...
func (w *WithDraw1ForContract) sign(key *ecdsa.PrivateKey) []byte {
	...
	buf.Write(utils.BigIntTo32Bytes(w.withdrawNonce))
...
func TestWithdraw(t *testing.T) {
	...
	log.Trace(fmt.Sprintf("WithDraw call, participant1=%s,participant2=%s,"+
		...
		"p1sig=0x%s,p2sig=0x%s,withdrawnonce=%s,channelid=%s",
...
//	tx, err := tokenNetwork.Unlock(auth, big.NewInt(12), auth.From, lockbytes)
//	if err != nil {
//		t.Error(err)
//		return
//	}
