...
import (
	...
	"github.com/SmartMeshFoundation/SmartRaiden/network/rpc/contractsv18/contracts"
...
func setup() {
	...
	tokenNetworkAddress = common.HexToAddress("0x16d79ebe2ae2b73cf8bc86550608d701ed17a176")
...
func creatAChannelAndDeposit(account1, account2 common.Address, key1 *ecdsa.PrivateKey, amount int64, conn *ethclient.Client) error {
	...
	_, settleBlock, _, state, err := tokenNetwork.GetChannelInfo(nil, account1, account2)
	if err != nil {
...
func TestCloseChannelAndUpdateNonClosingAndSettle(t *testing.T) {
	...
	tx, err = tokenNetwork.UpdateNonClosingBalanceProof(auth, auth.From, partnerAddr, bp2.LocksRoot, bp2.TransferAmount, bp2.Nonce, bp2.AdditionalHash, bp2.Signature, bp2.NonClosingSignature)
	if err != nil {
		t.Error(err)
		return
	}
...
func TestCloseChannelAndUpdateNonClosingAndSettle2(t *testing.T) {
	...
	//第二次 update, 应该也要成功
	tx, err = tokenNetwork.UpdateNonClosingBalanceProof(auth, auth.From, partnerAddr, bp3.LocksRoot, bp3.TransferAmount, bp3.Nonce, bp3.AdditionalHash, bp3.Signature, bp3.NonClosingSignature)
	if err != nil {
		t.Error(err)
		return
	}
...
func TestUnlock(t *testing.T) {
	...
	_, locksroot, transferAmount, _, _, err := tokenNetwork.GetChannelParticipantInfo(nil, auth.From, partnerAddr)
	if err != nil {
		t.Error(err)
		return
	}
	...
	tx, err = tokenNetwork.Unlock(auth, auth.From, partnerAddr, lockbytes)
	if err != nil {
		t.Error(err)
		return
	}
...
func TestPunishObsoleteUnlock(t *testing.T) {
	...
	tx, err = tokenNetwork.PunishObsoleteUnlock(auth, auth.From, partnerAddr, lockhash, ou.AdditionalHash,
