...
export class SwapDialogComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.tokenAddressMapping$ = this.smartraidenService.getTokens()
            .map((userTokens) => this.tokenPipe.tokensToSelectItems(userTokens))
            .share();

        this.form = this.fb.group({
            partner_address: [null, (control) =>
                control.value === this.smartraidenService.smartraidenAddress ? { ownAddress: true } : undefined],
            identifier: null,
            role: 'maker',
            sending_token: null,
        ...
        this.formString$ = this.form.valueChanges
            ...
            .map(([value, status]) => {
                ...
                const data = [
                    ...
                    this.smartraidenService.smartraidenAddress,
                    value.identifier,
                ...
                const hash = this.smartraidenService.sha3(data);
    ...
    public accept() {
        ...
        this.smartraidenService.swapTokens(value)
            .subscribe(() => {
    ...
    public parseFormString(formString: string) {
        ...
        if (dataarray.length !== 7 || formString.slice(-2) !== hash.slice(-2)) {
            return;
        } else if (dataarray[0].toLowerCase() !== this.smartraidenService.smartraidenAddress.toLowerCase()) {
            this.sharedService.msg({
                severity: 'warn',
