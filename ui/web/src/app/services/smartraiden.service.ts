...
import { SmartRaidenConfig } from './smartraiden.config';
...
@Injectable()
export class SmartRaidenService {

    public tokenContract: any;
    public smartraidenAddress: string;
    ...
    constructor(
        ...
        this.tokenContract = this.smartraidenConfig.web3.eth.contract(tokenabi);
    }
    ...
    get blockNumber(): number {
        return this.smartraidenConfig.web3.eth.blockNumber;
    }

    getBlockNumber(): Observable<number> {
        return Observable.bindNodeCallback((cb: CallbackFunc) =>
            this.smartraidenConfig.web3.eth.getBlockNumber(this.zoneEncap(cb)))();
    }

    public getSmartRaidenAddress(): Observable<string> {
        return this.http.get<{ our_address: string }>(`${this.smartraidenConfig.api}/address`)
            .map((data) => this.smartraidenAddress = data.our_address)
            .catch((error) => this.handleError(error));
    }

    public getChannels(): Observable<Array<Channel>> {
        return this.http.get<Array<Channel>>(`${this.smartraidenConfig.api}/channels`)
            .catch((error) => this.handleError(error));
    }

    public getTokens(refresh: boolean = false): Observable<Array<Usertoken>> {
        return this.http.get<Array<string>>(`${this.smartraidenConfig.api}/tokens`)
            .combineLatest(refresh ?
                this.http.get<Connections>(`${this.smartraidenConfig.api}/connections`) :
                Observable.of(null))
    ...
    public openChannel(
        ...
        return this.http.put<Channel>(`${this.smartraidenConfig.api}/channels`, data)
            .catch((error) => this.handleError(error));
    }
    ...
    public initiateTransfer(
        ...
        console.log(`${this.smartraidenConfig.api}/transfers/${tokenAddress}/${partnerAddress}`);
        return this.http.post(`${this.smartraidenConfig.api}/transfers/${tokenAddress}/${partnerAddress}`, data)
            .catch((error) => this.handleError(error));
    }
    ...
    public depositToChannel(channelAddress: string, balance: number): Observable<any> {
        ...
        return this.http.patch(`${this.smartraidenConfig.api}/channels/${channelAddress}`, data)
            .catch((error) => this.handleError(error));
    }
    ...
    public registerToken(tokenAddress: string): Observable<Usertoken> {
        return this.http.put(`${this.smartraidenConfig.api}/tokens/${tokenAddress}`, {})
    ...
    public connectTokenNetwork(funds: number, tokenAddress: string): Observable<any> {
        ...
        return this.http.put(`${this.smartraidenConfig.api}/connections/${tokenAddress}`, data)
            .catch((error) => this.handleError(error));
    }

    public leaveTokenNetwork(tokenAddress: string): Observable<any> {
        return this.http.delete(`${this.smartraidenConfig.api}/connections/${tokenAddress}`)
            .catch((error) => this.handleError(error));
    }
    ...
    public getEvents(
        ...
        return this.http.get<Array<Event>>(`${this.smartraidenConfig.api}/events/${path}`, { params })
            .catch((error) => this.handleError(error));
    }
    ...
    public swapTokens(swap: SwapToken): Observable<boolean> {
        ...
        return this.http.put(`${this.smartraidenConfig.api}/token_swaps/${swap.partner_address}/${swap.identifier}`,
    ...
    public sha3(data: string): string {
        return this.smartraidenConfig.web3.sha3(data, { encoding: 'hex' });
    }

    public blocknumberToDate(block: number): Observable<Date> {
        return Observable.bindNodeCallback((b: number, cb: CallbackFunc) =>
            this.smartraidenConfig.web3.eth.getBlock(b, this.zoneEncap(cb)))(block)
