# Helper guide to install and test the SmartRaiden UI

* Once you come in the `smartraiden/ui/web/` folder please run

> **npm install**.

This will install all the dependencies needed by Angular to run the project.

* We make Cross domain requests to many servers including the Geth Server and SmartRaiden API server we need to proxy our requests hence run the server in this way.
...
* If we run geth and smartraiden locally these are additional configurations that we need to do on our local machine.
	...
	4. Start SmartRaiden with the registry-contract-address and the discovery-contract-address generated in the last lines in the **mycustomgenesis.json**
file.
	```
    smartraiden --eth-rpc-endpoint 127.0.0.1:8545 --registry-contract-address 7d73424a8256c0b2ba245e5d5a3de8820e45f390 --discovery-contract-address 08425d9df219f93d5763c3e85204cb5b4ce33aaa --keystore-path ~/privategeth/keystore --console
    ```
