...
func testNewToken(t *testing.T, ra, rb, rc, rd *RaidenApi) (tokenAddr common.Address) {
	...
	assert(t, token.Transfer(rb.Raiden.NodeAddress, 500), nil)
	assert(t, token.Transfer(rc.Raiden.NodeAddress, 500), nil)
	assert(t, token.Transfer(rd.Raiden.NodeAddress, 500), nil)
...
func testCreateChannel(t *testing.T, tokenAddr common.Address, contractBalance int64, ra, rb, rc *RaidenApi) {
	var err error
...
func TestSmoke(t *testing.T) {
	...
	assert(t, ra.Raiden.GetChannel(tokenAddr, rb.Raiden.NodeAddress).Balance(), contractBalance-tAmount)
	assert(t, rb.Raiden.GetChannel(tokenAddr, ra.Raiden.NodeAddress).Balance(), contractBalance+tAmount)
	...
	//channel a-b of tokenaddr
	assert(t, ra.Raiden.GetChannel(tokenAddr, rb.Raiden.NodeAddress).Balance(), contractBalance-tAmount*2)
	assert(t, rb.Raiden.GetChannel(tokenAddr, ra.Raiden.NodeAddress).Balance(), contractBalance+2*tAmount)
	//channel b-c of tokenaddr
	assert(t, rb.Raiden.GetChannel(tokenAddr, rc.Raiden.NodeAddress).Balance(), contractBalance-tAmount)
	assert(t, rc.Raiden.GetChannel(tokenAddr, rb.Raiden.NodeAddress).Balance(), contractBalance+tAmount)

	log.Info("step 4 D connect to this token network")
	if true {
		err = rd.ConnectTokenNetwork(tokenAddr, 300, 3, 0.4)
		if err != nil {
			t.Error(err)
			return
		}
	...
	err = rb.ExpectTokenSwap(32, tokenAddr, tokenAddr2, ra.Raiden.NodeAddress, rb.Raiden.NodeAddress, tAmount, 2*tAmount)
	if err != nil {
		t.Error(err)
		return
	}
	err = ra.TokenSwapAndWait(32, tokenAddr, tokenAddr2, ra.Raiden.NodeAddress, rb.Raiden.NodeAddress, tAmount, 2*tAmount)
	if err != nil {
		t.Error(err)
		return
	}
	...
	assert(t, ra.Raiden.GetChannel(tokenAddr, rb.Raiden.NodeAddress).Balance(), contractBalance-3*tAmount)
	assert(t, rb.Raiden.GetChannel(tokenAddr, ra.Raiden.NodeAddress).Balance(), contractBalance+3*tAmount)

	//channel a-b of tokenadd4 a+amount*2 b-amount*2
	assert(t, ra.Raiden.GetChannel(tokenAddr2, rb.Raiden.NodeAddress).Balance(), contractBalance+2*tAmount)
	assert(t, rb.Raiden.GetChannel(tokenAddr2, ra.Raiden.NodeAddress).Balance(), contractBalance-2*tAmount)

	log.Info(" step 6 make a token swap between A and c through b")
	err = rc.ExpectTokenSwap(33, tokenAddr, tokenAddr2, ra.Raiden.NodeAddress, rc.Raiden.NodeAddress, tAmount, 2*tAmount)
	if err != nil {
		t.Error(err)
		return
	}
	err = ra.TokenSwapAndWait(33, tokenAddr, tokenAddr2, ra.Raiden.NodeAddress, rc.Raiden.NodeAddress, tAmount, 2*tAmount)
	if err != nil {
		t.Error(err)
		return
	}
	...
	//channel a-b of tokenaddr a-amount b+amount
	assert(t, ra.Raiden.GetChannel(tokenAddr, rb.Raiden.NodeAddress).Balance(), contractBalance-4*tAmount)
	assert(t, rb.Raiden.GetChannel(tokenAddr, ra.Raiden.NodeAddress).Balance(), contractBalance+4*tAmount)
	//channel b-c of tokenaddr b-amount c+amount
	assert(t, rb.Raiden.GetChannel(tokenAddr, rc.Raiden.NodeAddress).Balance(), contractBalance-2*tAmount)
	assert(t, rc.Raiden.GetChannel(tokenAddr, rb.Raiden.NodeAddress).Balance(), contractBalance+2*tAmount)

	//channel a-b of tokenaddr2 a+2*amount b-2*amount
	assert(t, ra.Raiden.GetChannel(tokenAddr2, rb.Raiden.NodeAddress).Balance(), contractBalance+4*tAmount)
	assert(t, rb.Raiden.GetChannel(tokenAddr2, ra.Raiden.NodeAddress).Balance(), contractBalance-4*tAmount)
	//channel b-c of tokenaddr2 b+2amount c-2*amount
	assert(t, rb.Raiden.GetChannel(tokenAddr2, rc.Raiden.NodeAddress).Balance(), contractBalance+2*tAmount)
	assert(t, rc.Raiden.GetChannel(tokenAddr2, rb.Raiden.NodeAddress).Balance(), contractBalance-2*tAmount)
