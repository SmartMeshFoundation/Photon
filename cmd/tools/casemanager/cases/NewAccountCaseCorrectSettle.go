...
func (cm *CaseManager) NewAccountCaseCorrectSettle() (err error) {
	...
	env, err := models.NewTestEnv("./cases/NewAccountCaseCorrectSettle.ENV", cm.UseMatrix, cm.EthEndPoint)
	if err != nil {
		return
	}
	defer func() {
		log.Trace(fmt.Sprintf("NewAccountCaseCorrectSettle err=%s", err))
		if env.Debug == false {
