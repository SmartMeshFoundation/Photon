...
func (cm *CaseManager) NewAccountCaseCannotUpdateBalanceProofAfterChannelClosed02() (err error) {
	...
	env, err := models.NewTestEnv("./cases/NewAccountCaseCannotUpdateBalanceProofAfterChannelClosed02.ENV", cm.UseMatrix, cm.EthEndPoint)
	if err != nil {
		return
	}
	defer func() {
		log.Trace(fmt.Sprintf("NewAccountCaseCannotUpdateBalanceProofAfterChannelClosed02 err=%s", err))
		if env.Debug == false {
