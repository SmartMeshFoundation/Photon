...
//put request
func (this *Api) OpenChannel(partnerAddress, tokenAddress string, settleTimeout, balance int) (channel string, err error) {
	...
	if err != nil {
		...
		if balance > 0 {
			err = this.api.Deposit(tokenAddr, partnerAddr, int64(balance), params.DEFAULT_POLL_TIMEOUT)
			if err == nil {
				d.Balance = c.Balance()
			} else {
...
func (this *Api) DepositChannel(channelAddres string, balance int64) (channel string, err error) {
	chAddr := common.HexToAddress(channelAddres)
	c, err := this.api.GetChannel(chAddr)
	if err != nil {
		log.Error(err.Error())
...
*/
func (this *Api) Transfers(tokenAddress, targetAddress string, amount int64, identifier uint64) (transfer string, err error) {
	tokenAddr := common.HexToAddress(tokenAddress)
...
role: "maker" or "taker"
*/
func (this *Api) TokenSwap(role string, Identifier, SendingAmount, ReceivingAmount int64, SendingToken, ReceivingToken, TargetAddress string) (err error) {
	type Req struct {
...
*/
//Connecting to a token network
func (this *Api) ConnectToTokenNetwork(tokenAddress string, funds int64) (err error) {
	token := common.HexToAddress(tokenAddress)
	if funds <= 0 {
