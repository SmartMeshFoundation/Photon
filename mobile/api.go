...
*/
func (a *API) Transfers(tokenAddress, targetAddress string, amountstr string, feestr string, id int64, isDirect bool) (transfer string, err error) {
	defer func() {
		log.Trace(fmt.Sprintf("Api Transfers tokenAddress=%s,targetAddress=%s,amountstr=%s,feestr=%s,id=%d,isDirect=%v,\nout transfer=\n%s,err=%v",
			tokenAddress, targetAddress, amountstr, feestr, id, isDirect, transfer, err,
		))
	}()
	tokenAddr := common.HexToAddress(tokenAddress)
	...
	err = a.api.Transfer(tokenAddr, amount, fee, targetAddr, identifier, params.MaxRequestTimeout, isDirect)
	if err != nil {
		log.Error(err.Error())
		return
	}
